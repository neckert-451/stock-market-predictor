...
#the purpose of this function is to compile all the data from the 500 s&p companies and put the data into one csv file
def compile_data():
    with open("sp500tickers.pickle", "rb") as f:
        tickers = pickle.load(f)
    ...
    for count, ticker in enumerate(tickers):
        try:
            df = pd.read_csv(f'stock_dfs/{ticker}.csv'.format(ticker))
            ...
            print(f'stock_dfs/{ticker}.csv'.format(ticker) + ' not found')
...
# comment out the execution of this function
compile_data()
...
for ticker in tickers:
    ...
    # write to CSV
    with open('output_file.csv', 'a', newline='\n') as csv_file:
